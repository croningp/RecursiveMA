...
def _build_tree(data, level=1, acc=None, parent=None, max_level=3):
    ...
    if level == 1:
        acc = {}
        for peak in data[1]['ms1_mz']:
            acc[peak] = {}
            _build_tree(data, level=2, acc=acc[peak], parent=peak, max_level=max_level)
        return acc
    level_df = data[level]
    level_df = level_df.join(data[level - 1].drop(columns=['parent_id'], errors='ignore'), on='parent_id')
    for peak in level_df[level_df[f'ms{level - 1}_mz'] == parent][f'ms{level}_mz']:
